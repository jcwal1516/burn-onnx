...
# used to generate model: onnx-tests/tests/range/range_negative_delta.onnx
# This test has a descending range with negative delta (start=10, limit=0, delta=-2)
...
def main():
    ...
    graph_def = helper.make_graph(
        ...
        name="RangeNegativeDeltaGraph",
        inputs=[],
        outputs=[helper.make_tensor_value_info("output", TensorProto.INT64, [None])],
    )

    model_def = helper.make_model(graph_def, producer_name="range_negative_delta")
    model_def.opset_import[0].version = 16

    # Save the model
    onnx.save(model_def, "range_negative_delta.onnx")
    print("Model saved to range_negative_delta.onnx")

    # Test with ReferenceEvaluator
    sess = ReferenceEvaluator(model_def)
    result = sess.run(None, {})
    expected = np.arange(10, 0, -2, dtype=np.int64)
