...
"""Download Inception v2 (BN-Inception) from the ONNX Model Zoo."""
...
# ONNX Model Zoo: Inception v2, opset 9
...
def download_model(output_path):
    """Download Inception v2 ONNX model from the ONNX Model Zoo."""
    print(f"Downloading Inception v2 from ONNX Model Zoo...")
...
def generate_test_data(model_path, output_dir):
    ...
    np.random.seed(42)
    # Inception v2 expects [1, 3, 224, 224]
...
def main():
    print("=" * 60)
    print("Inception v2 (BN-Inception) Model Preparation")
    print("=" * 60)

    artifacts_dir = get_artifacts_dir(MODEL_NAME)

    raw_path = artifacts_dir / "inception_v2_raw.onnx"
    model_path = artifacts_dir / "inception_v2.onnx"
    ...
    print("\n" + "=" * 60)
    print(f"  Inception v2 model preparation completed!")
    print(f"  Model: {model_path}")
    print(f"  Test data: {test_data_path}")
    print("=" * 60)
