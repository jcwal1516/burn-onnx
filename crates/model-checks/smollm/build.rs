...
fn main() {
    ...
    let model_name = env::var("SMOLLM_MODEL").unwrap_or_else(|_| {
        ...
        eprintln!();
        eprintln!("Please specify which SmolLM model to build:");
        eprintln!("  SMOLLM_MODEL=smollm-135m cargo build");
        eprintln!();
    ...
    let artifacts = model_checks_common::artifacts_dir_build("smollm");
    ...
    println!("cargo:rerun-if-env-changed=SMOLLM_MODEL");
